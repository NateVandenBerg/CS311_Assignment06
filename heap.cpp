...
T Heap::getMax() const {//finds max value in array
    // Initialize largest as root
    if (count == 0) {
        return -1;
    }
    int largest = heapArray[getMaxIndex(0)];

    return largest;
}

T Heap::getMaxIndex(int index) const {//finds maxValue index Location
    //cout << "Max I::";
                                      // Initialize largest as root
    int largest = index;
    ...
    if (l < count && heapArray[l] > heapArray[largest] && heapArray[l] > heapArray[r]) {
        largest = l;
        getMaxIndex(l);
    }
    ...
    // so far
    if (r < count && heapArray[r] > heapArray[largest] && heapArray[r] > heapArray[l]) {
        largest = r;
        getMaxIndex(r);
    }
